 ...
 * var resultData = {
 ...
 协定协议:rock://class:port/method?params;
 params是一串json字符串
 */
(function () {
    var doc = document;
    ...
    var JS_BRIDGE_PROTOCOL_SCHEMA = "rock";
    var increase = 1;
    var RockBridge = win.RockBridge || (win.RockBridge = {});
    ...
    for (var index in ExposeMethod) {
        if (ExposeMethod.hasOwnProperty(index)) {
            if (!Object.prototype.hasOwnProperty.call(RockBridge, index)) {
                RockBridge[index] = ExposeMethod[index];
            }
        }
    }
})();
